...
def cross_val(fold_num, lambda_list, num_iters, k_fold_data, y_data, k_fold_indices, conv_tol):
    ...
    for fold_int in range(0, fold_num):
        for lambda_value in range(0,param_num):
            ...
            theta, train_errors= sols_grad_desc.stoch_linear_calc(X_train, y_train, theta, sel_lambda, conv_tol, num_iters, iterations_single_lambda[lambda_value])

            #calculate error on test set
            test_error = sols_grad_desc.cost_linear_reg(X_test, y_test, theta)
...
def AUC_curve(min_pred,min_y_test):
    ...
    print('The Area Under Curve is: ', 1/2+final_auc)

    ax = plt.subplot(111)
    #we plot y- true positives against x- false positives
    ax.plot(false_rates, true_rates,color='r', label = ('ROC-Curve with AUC =' + str(1/2+final_auc)))
    box = ax.get_position()
...
def main():
    ...
    # generating plot for all lambdas so we have 3 differest test errors
